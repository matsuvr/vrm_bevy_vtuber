//! Compatibility runner for VRM 1.0 fixtures.
//!
...
pub struct CompatibilityResult {
    /// Path to the model.
    pub path: PathBuf,
    /// Preflight summary, if the file is a valid VRM 1.0 model.
...
pub fn run(fixture_dir: impl AsRef<Path>) -> Result<Vec<CompatibilityResult>, String> {
    ...
    let mut entries: Vec<PathBuf> = std::fs::read_dir(fixture_dir)
        ...
        .filter_map(|e| e.ok())
        .map(|e| e.path())
        .filter(|p| p.extension().and_then(|s| s.to_str()) == Some("vrm"))
        .collect();
    entries.sort();

    let mut results = Vec::with_capacity(entries.len());
    for path in entries {
        let result = run_single(&path)?;
        results.push(result);
    }
    Ok(results)
}
