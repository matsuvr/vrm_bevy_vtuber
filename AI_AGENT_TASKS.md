...
#### Q2-06-004: T-poseを避けるmodel-adaptive arm defaultを適用する
...
- Issue #14で完全な`upperArm`／`lowerArm`／`hand` chainとrest-space geometryをcacheし、shoulder／fingerはoptional capabilityとして保持する。
- Issue #15のpure analytic two-bone IKでmodel-adaptiveなtyped default arm poseを解決する。既定profileはdrop 70°、reach 0.99、forward `+Z` offset 0.081 total、rearward `-Z` elbow pole 0.05 totalとする。
- Issue #16でbinding-timeの固定±55° `Transform`書き込みを撤去し、AnimationSystemsとdirect-pose BodyTracking後のcompositorでrest-relative deltaを`base * delta`合成する。実際の`ChildOf`経路と中間nodeを含むGlobalTransformを更新する。
- `RestTransform`／`RestGlobalTransform`を変更せず、完全chainのないside、generation不一致、replacement、退化geometryは安全なno-opとする。head〜spineのBodyTracking writer所有権、eye gaze、Node Constraint、SpringBoneは変更しない。

**完了条件**

- rest-space chain binding、IKのclamp／pole／non-identity rest、compositorのnon-accumulation、small animation update、intermediate ChildOf propagation、generation replacement、rest pose不変が自動testで固定される。
- workspace fmt、check、clippy、対象test、`git diff --check`が成功する。
- Windows／macOS実VRMでのvisual確認は、実施しなければ`NOT RUN`と明記する。

**完了記録（2026-08-13）**

- bindingがcomplete arm chainを一度だけ解決し、rest-space geometryからanalytic IK poseをgeneration付きでcacheする。`RestTransform`／`RestGlobalTransform`は不変である。
